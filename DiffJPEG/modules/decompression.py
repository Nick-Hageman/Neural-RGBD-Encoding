...
#             cb = repeat(cb)
#             cr = repeat(cr)
            cb = torch.squeeze(nn.Upsample(scale_factor=2, mode='bilinear')(torch.unsqueeze(cb, 0)), 0)
            cr = torch.squeeze(nn.Upsample(scale_factor=2, mode='bilinear')(torch.unsqueeze(cr, 0)), 0)
#             cb = torch.squeeze(torch.nn.functional.interpolate(torch.unsqueeze(cb, 0), scale_factor = 2, mode ='bilinear', antialias=True), 0)
#             cr = torch.squeeze(torch.nn.functional.interpolate(torch.unsqueeze(cr, 0), scale_factor = 2, mode ='bilinear', antialias=True), 0)
...
class decompress_jpeg(nn.Module):
    ...
    def __init__(self, height, width, subsampling=True):
        ...
        self.chroma = chroma_upsampling(subsampling)
    ...
    def forward(self, y, cb, cr, factor, rounding):
        ...
        for k in components.keys():
            if k in ('cb', 'cr') and self.subsampling:
                #comp = components[k] #edit
...
#         image = torch.clamp(rounding(image * 255) / 255, min=0, max=1)
#         return image
