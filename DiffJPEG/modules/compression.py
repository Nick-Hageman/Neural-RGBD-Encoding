...
#             cb = torch.nn.functional.interpolate(image_2[:, 1, :, :].unsqueeze(1), scale_factor = 0.5, mode ='bilinear', antialias=True)
#             cr = torch.nn.functional.interpolate(image_2[:, 2, :, :].unsqueeze(1), scale_factor = 0.5, mode ='bilinear', antialias=True)
...
class compress_jpeg(nn.Module):
    ...
    """
    def __init__(self, subsampling=True):
    ...
    def forward(self, image, factor, rounding):
        ...
        for k in components.keys():
            ...
            if k in ('cb', 'cr') and self.subsampling: #edit
                comp = self.c_quantize(comp, factor, rounding)
            else:
                comp = self.y_quantize(comp, factor, rounding)
